document.addEventListener('DOMContentLoaded', function () {
    ...
    function showRanking() {
        ...
        // Cerrar el popup después de 5 segundos (5000 ms)
        setTimeout(() => {
            closeRanking();
        }, 0.1); // Ajusta el tiempo según tus necesidades
    }
    ...
    // Llamar a renderRanking y mostrar el ranking al cargar la página
    renderRanking();

    // Verificar si se debe mostrar el ranking popup al cargar la página
    if (localStorage.getItem('rankingShown') !== 'true') {
        showRanking();
        localStorage.setItem('rankingShown', 'true'); // Marcar el ranking como mostrado
    }
